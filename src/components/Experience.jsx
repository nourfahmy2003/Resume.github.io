...
export default function Experience(){
  ...
  useEffect(()=>{
    ...
    if(!root) return;
    const timeline = root.querySelector('#experience-timeline');
    const list = root.querySelector('.timeline-list');
    if(!timeline || !list) return;

    const indicator = root.querySelector('.timeline-indicator');
    indicator.innerHTML = '';
    ...
    items.forEach((li, idx)=>{
      ...
      dot.setAttribute('data-date', li.dataset.date || '');
      dot.addEventListener('click', ()=> li.scrollIntoView({ behavior:'smooth', block:'start' }));
    ...
    const onScroll = ()=>{
      ...
      items.forEach((li, i)=>{
        ...
        const delta = Math.abs(r.top - window.innerHeight*0.35);
        if (delta < minDelta){ minDelta = delta; active = i; }
        if (r.top < window.innerHeight*0.85) li.classList.add('visible');
      });
      indicator.querySelectorAll('.indicator-dot').forEach((d,i)=> d.classList.toggle('active', i===active));
  ...
  return (
    <section className="experience-section section-container fade-section" id="experience" ref={rootRef}>
